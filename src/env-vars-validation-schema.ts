...
export const envVarsValidationSchema = Joi.object({
  ...
  PUBLIC_RESOLVER_ADDRESS: Joi.string().required(),
  ENS_REGISTRY_ADDRESS: Joi.string().required(),
  DID_REGISTRY_ADDRESS: Joi.string().required(),
  DOMAIN_NOTIFIER_ADDRESS: Joi.string().required(),
  RESOLVER_V1_ADDRESS: Joi.string().required(),
  RESOLVER_V2_ADDRESS: Joi.string().required(),
  CHAIN_REQUEST_ATTEMPTS: Joi.number().positive(),

  ASSETS_MANAGER_ADDRESS: Joi.string().required(),
